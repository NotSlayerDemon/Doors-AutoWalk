...
game:GetService("RunService").RenderStepped:connect(function()
    ...
    LocalPlayer.Character.Humanoid.WalkSpeed = 15 -- Normal Walkspeed is 21
    ...
    if Entity then
        if Path then
            if Path.Parent.Name == "Rooms_Locker" then
                if Entity.Main.Position.Y > -4 then
                    if (LocalPlayer.Character.HumanoidRootPart.Position - Path.Position).Magnitude < 1.5 then
        ...
        if Entity.Main.Position.Y < -4 then -- Instead of waiting for them to be deleted just do this LOL
...
while true do
    ...
    local path = PathfindingService:CreatePath({ WaypointSpacing = 1, AgentRadius = 0.5, AgentCanJump = false })
